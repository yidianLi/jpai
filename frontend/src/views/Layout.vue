<template>
  <el-container class="layout-shell">
    <el-aside class="sidebar" :width="collapsed ? '76px' : '248px'" :class="{ collapsed }">
      <div class="brand">
        <div class="brand-mark"><el-icon><Cpu /></el-icon></div>
        <div v-if="!collapsed"><strong>简普数智资产管理后台</strong><span>Asset Management Console</span></div>
      </div>
      <div v-if="!collapsed" class="workspace-label">工作台</div>
      <el-menu class="nav-menu" :collapse="collapsed" :default-active="route.path" router>
      ...
      <div v-if="!collapsed" class="sidebar-footer">资产运营中心 · {{ currentDate }}</div>
    </el-aside>
    <el-container>
      <el-header class="topbar">
        <div class="topbar-title"><el-button circle text @click="collapsed = !collapsed" :aria-label="collapsed ? '展开导航' : '收起导航'"><el-icon><Expand v-if="collapsed"/><Fold v-else/></el-icon></el-button><div><div class="eyebrow">资产管理平台</div><h1>{{ currentTitle }}</h1></div></div>
...
import { Cpu, ArrowDown, DataAnalysis, CircleCheck, RefreshRight, Files, Delete, Search, Setting, Fold, Expand } from '@element-plus/icons-vue'
...
.brand { border-bottom-color:#e7eef6; }.brand-mark { width:36px; height:36px; display:grid; place-items:center; color:#1769aa; background:#eaf4fc; border-radius:10px; font-size:20px; flex:0 0 36px; }.brand strong { display:block; font-size:17px; color:#20334d; letter-spacing:.5px; }.brand span { display:block; margin-top:3px; color:#8191a7; font-size:10px; }
...
.topbar { height:76px; display:flex; align-items:center; justify-content:space-between; padding:0 30px; background:#fff; border-bottom:1px solid #e6edf5; }.topbar-title { display:flex; align-items:center; gap:10px; }.eyebrow { color:#8191a7; font-size:11px; }.topbar h1 { margin:3px 0 0; color:#20334d; font-size:21px; }.topbar-actions { display:flex; align-items:center; gap:8px; color:#53657d; font-size:13px; }.status-dot { width:7px; height:7px; border-radius:50%; background:#20b486; }.status-text { color:#198f6b; }.user-info { display:flex; align-items:center; gap:8px; cursor:pointer; }.main-content { padding:28px 30px; overflow:auto; }
.sidebar.collapsed .brand { justify-content:center; padding-left:12px; padding-right:12px; }.sidebar.collapsed .nav-menu { padding-left:10px; padding-right:10px; }.sidebar.collapsed :deep(.el-menu-item), .sidebar.collapsed :deep(.el-sub-menu__title) { justify-content:center; padding-left:0!important; padding-right:0!important; }.sidebar.collapsed :deep(.el-menu-item span), .sidebar.collapsed :deep(.el-sub-menu__title span), .sidebar.collapsed :deep(.el-sub-menu__icon-arrow) { display:none; }
</style>
